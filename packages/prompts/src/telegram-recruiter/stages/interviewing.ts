...
export function buildInterviewingPrompt(
  ...
  let voiceTaskText = "";
  if (!hasFirstVoice) {
    // ЭТАП 1: Просим первое голосовое - ОРГАНИЗАЦИОННЫЕ вопросы
    voiceTaskText = `
⚠️ ЭТАП 1/2: ОРГАНИЗАЦИОННЫЕ ВОПРОСЫ
...
- Будь краток: 2-3 предложения максимум`;
    ...
    // ЭТАП 2: Просим второе голосовое - ТЕХНИЧЕСКИЕ/ПРОФЕССИОНАЛЬНЫЕ вопросы
    voiceTaskText = `
⚠️ ЭТАП 2/2: ТЕХНИЧЕСКИЕ ВОПРОСЫ (ПОСЛЕДНЕЕ ГОЛОСОВОЕ)

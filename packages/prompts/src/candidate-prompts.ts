...
export function buildTelegramInvitePrompt(
  ...
  return `Ты — рекрутер компании "${companyName}". Напиши короткое персонализированное сообщение кандидату с приглашением продолжить общение в Telegram.
...
- НЕ упоминай оценки, скрининг или автоматизацию
- НЕ используй шаблонные фразы типа "рады сообщить", "благодарим за отклик"
- В КОНЦЕ обязательно предложи продолжить общение в Telegram
- Объясни кратко, почему в Telegram удобнее (голосовые, быстрее отвечать)
...
- Пиши как в обычной переписке: просто, по-человечески
- Используй эмодзи умеренно (1-2 максимум)
- Длина: 2-3 короткие предложения + приглашение в Telegram

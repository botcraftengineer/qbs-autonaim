...
export const messageBufferedDataSchema = z.object({
  ...
  interviewStep: z.number().int().min(0, "Шаг интервью должен быть неотрицательным"),
  messageId: z.string().min(1, "ID сообщения обязателен"),
  timestamp: z.number().int().positive("Временная метка должна быть положительной"),
});
...
export const bufferFlushDataSchema = z.object({
  ...
  messageCount: z.number().int().min(0, "Количество сообщений должно быть неотрицательным"),
});

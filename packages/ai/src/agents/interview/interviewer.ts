/**
 * Агент для проведения интервью
 */

import { z } from "zod";
import { extractFirstName } from "../../utils/name-extractor";
import { wrapUserContent } from "../../utils/sanitize";
import { type AgentConfig, BaseAgent } from "../core/base-agent";
import { RECRUITER_PERSONA } from "../core/persona";
import { getConversationContext, getVoiceMessagesInfo } from "../core/tools";
import { AgentType, type BaseAgentContext } from "../core/types";

export interface InterviewerInput {
  questionNumber: number;
  customOrganizationalQuestions?: string | null;
  customInterviewQuestions?: string | null; // Технические вопросы
  resumeLanguage?: string;
}

const interviewerOutputSchema = z.object({
  analysis: z.string(),
  shouldContinue: z.boolean(),
  reason: z.string(),
  nextQuestion: z.string(),
  confidence: z.number().min(0).max(1),
  waitingForCandidateResponse: z.boolean(),
  isSimpleAcknowledgment: z.boolean(),
});

export type InterviewerOutput = z.infer<typeof interviewerOutputSchema>;

export class InterviewerAgent extends BaseAgent<
  InterviewerInput,
  InterviewerOutput
> {
  constructor(config: AgentConfig) {
    const instructions = `${RECRUITER_PERSONA.INSTRUCTIONS}

ТВОЯ ЗАДАЧА:
- Проанализируй ответ кандидата на последний вопрос.
- Сформулируй следующий уточняющий или новый вопрос.
- Если есть организационные темы — задавай вопросы о графике, зарплате, локации.
- Если есть технические темы — углубляйся в опыт и навыки.
- Для gig-интервью (разовых задач) НЕ задавай организационные вопросы, фокусируйся на профессиональных навыках.
- Используй инструменты для анализа истории, если это необходимо.

${RECRUITER_PERSONA.GREETING_RULES}`;

    super(
      "Interviewer",
      AgentType.INTERVIEWER,
      instructions,
      interviewerOutputSchema,
      {
        ...config,
        tools: {
          getVoiceMessagesInfo,
          getConversationContext,
        },
      },
    );
  }

  protected validate(input: InterviewerInput): boolean {
    // Проверяем обязательные поля
    if (!Number.isFinite(input.questionNumber) || input.questionNumber < 0) {
      console.error(
        "[InterviewerAgent] Invalid questionNumber:",
        JSON.stringify({
          questionNumber: input.questionNumber,
          type: typeof input.questionNumber,
        }),
      );
      return false;
    }

    // Логируем успешную валидацию для отладки
    console.log("[InterviewerAgent] Validation passed:", {
      questionNumber: input.questionNumber,
    });

    return true;
  }

  protected buildPrompt(
    input: InterviewerInput,
    context: BaseAgentContext,
  ): string {
    const { candidateName, vacancyTitle, vacancyDescription } = context;
    const { resumeLanguage = "ru" } = input;

    const name = extractFirstName(candidateName || null);
    const botName = context.companySettings?.botName || "Дмитрий";
    const botRole = context.companySettings?.botRole || "рекрутер";
    const companyName = context.companySettings?.name || "";

    // Передаём полную историю диалога без обрезки
    const historyText =
      context.conversationHistory.length > 0
        ? context.conversationHistory
            .map((msg) => {
              const sender = msg.sender === "CANDIDATE" ? "К" : "Я";
              return `${sender}: ${msg.content}`;
            })
            .join("\n")
        : "";

    // Определяем тип интервью по наличию организационных вопросов
    // Если customOrganizationalQuestions === null, это может быть gig без орг. вопросов
    const isGigInterview = input.customOrganizationalQuestions === null;

    // Передаём кастомные вопросы без обрезки
    const organizationalQuestionsBlock = input.customOrganizationalQuestions
      ? wrapUserContent(
          input.customOrganizationalQuestions,
          "organizational-questions",
          "ОРГАНИЗАЦИОННЫЕ ТЕМЫ:",
        )
      : "";

    const technicalQuestionsBlock = input.customInterviewQuestions
      ? wrapUserContent(
          input.customInterviewQuestions,
          "technical-questions",
          "ТЕХНИЧЕСКИЕ ТЕМЫ:",
        )
      : "";

    // Передаём описание вакансии без обрезки
    const shortVacancyDesc = vacancyDescription || "";

    // Определяем логику вопросов в зависимости от типа интервью
    const interviewLogic = isGigInterview
      ? `Логика интервью (GIG - разовая задача):
1. Анализируй историю диалога, чтобы понять контекст
2. Не повторяй вопросы из истории
3. Задавай ТОЛЬКО профессиональные/технические вопросы
4. НЕ спрашивай про график работы, зарплату, формат работы - это разовая задача
5. Фокусируйся на опыте, навыках, портфолио
6. Если ответ полный - переходи к следующей теме
7. Если собрал достаточно - завершай (shouldContinue: false)`
      : `Логика интервью (вакансия):
1. Анализируй историю диалога, чтобы понять контекст
2. Не повторяй вопросы из истории
3. Сначала организационные темы, потом технические
4. Если ответ полный - переходи к следующей теме
5. Если собрал достаточно - завершай (shouldContinue: false)`;

    const firstInteractionLogic = isGigInterview
      ? `a) ПЕРВОЕ ВЗАИМОДЕЙСТВИЕ (question_number=1):
   - Кандидат ответил на приветствие
   → Задай 1-2 первых профессиональных вопроса естественно (про опыт, навыки, портфолио)
   → НЕ спрашивай про график, зарплату - это разовая задача
   → Упомяни про голосовые ненавязчиво: "Можете ответить голосовым, если удобно 🎤"
   → analysis: "Кандидат готов к интервью"
   → shouldContinue: true
   → nextQuestion: короткий вопрос (например: "Расскажите о вашем опыте в этой области?")`
      : `a) ПЕРВОЕ ВЗАИМОДЕЙСТВИЕ (question_number=1):
   - Кандидат ответил на приветствие
   → Задай 1-2 первых организационных вопроса естественно
   → Упомяни про голосовые ненавязчиво: "Можете ответить голосовым, если удобно 🎤"
   → analysis: "Кандидат готов к интервью"
   → shouldContinue: true
   → nextQuestion: короткий вопрос (например: "Какой график работы вам подходит?")`;

    return `КОНТЕКСТ:
Язык резюме: <resume_language>${resumeLanguage}</resume_language>
Адаптируйся к языку ответов кандидата.

Бот: <bot_name>${botName}</bot_name>, <bot_role>${botRole}</bot_role>${companyName ? `, <company>${companyName}</company>` : ""}
Кандидат: <candidate_name>${name}</candidate_name>
Вакансия: <vacancy_title>${vacancyTitle || "не указана"}</vacancy_title>${shortVacancyDesc ? `\n<vacancy_description>${shortVacancyDesc}</vacancy_description>` : ""}

${organizationalQuestionsBlock}
${technicalQuestionsBlock}

${historyText ? `ИСТОРИЯ ДИАЛОГА:\n${historyText}\n` : ""}

ТЕКУЩЕЕ ВЗАИМОДЕЙСТВИЕ:
Номер вопроса: <question_number>${input.questionNumber}</question_number>
Первое взаимодействие: <is_first_interaction>${input.questionNumber === 1}</is_first_interaction>
Анализируй последнее сообщение кандидата из истории диалога.

ПРАВИЛА:

${interviewLogic}

⚠️ СТИЛЬ ОБЩЕНИЯ:
- Пиши КОРОТКО и естественно, как живой человек в мессенджере
- Задавай 1-2 вопроса за раз, не больше
- СТРОГО ЗАПРЕЩЕНО: нумерация вопросов ("Вопрос 1:", "1.", "2."), списки, маркеры
- СТРОГО ЗАПРЕЩЕНО: комментарии в скобках типа "(Я слушаю ваш ответ...)", "(чтобы оценить...)"
- СТРОГО ЗАПРЕЩЕНО: оценочные комментарии типа "Отлично!", "Интересный подход", "Это показывает системность"
- СТРОГО ЗАПРЕЩЕНО: метаинформация о процессе интервью
- Используй простые фразы, эмодзи в меру (1-2 максимум)
- Пример ХОРОШЕГО: "Какой график работы вам подходит?"
- Пример ПЛОХОГО: "Отлично! Это показывает системность. А какой график работы вам подходит?"
- Пиши как реальный рекрутер в Telegram, а не как робот

ПРИМЕРЫ ПЛОХИХ СООБЩЕНИЙ (НИКОГДА НЕ ДЕЛАЙ ТАК):
❌ "Отлично, начнем. Вопрос 1: Опыт и подход. Расскажите о вашем опыте в дизайне. (Я слушаю ваш ответ, чтобы оценить глубину опыта)"
❌ "Теперь несколько вопросов: 1. График работы 2. Зарплата 3. Локация"
❌ "Вопрос 2: Расскажите о проектах (для оценки навыков)"
❌ "Интересный подход! Это показывает системность. А какой у вас опыт?"
❌ "Отлично! Хорошо видно вашу экспертизу. Расскажите подробнее?"
✅ "Расскажите о вашем опыте в дизайне мобильных приложений?"
✅ "Какой график работы вам подходит?"

⚠️ ГОЛОСОВЫЕ СООБЩЕНИЯ:
- Упоминай голосовые ТОЛЬКО ОДИН РАЗ в самом начале (question_number=1)
- Формулировка: "Можете ответить голосовым, если удобно 🎤" (ненавязчиво, как опция)
- Если кандидат отвечает текстом - продолжай текстом, БОЛЬШЕ НЕ упоминай голосовые
- Если кандидат говорит "не могу записать голосовое" → "Без проблем 🙂" и продолжай текстом

ЗАДАЧА:
Проанализируй ситуацию и определи действие:

${firstInteractionLogic}

b) ОТКАЗ ОТ ГОЛОСОВЫХ ("не могу записать", "нет микрофона", "только текстом"):
   → Спокойно: "Без проблем 🙂"
   → Продолжай текстом, НЕ упоминай голосовые больше
   → shouldContinue: true
   → nextQuestion: следующий вопрос

c) Простое подтверждение ("ок", "спасибо"):
   → shouldContinue: false
   → isSimpleAcknowledgment: true
   → nextQuestion: пустая строка

d) Вопрос/просьба отложить:
   → shouldContinue: true
   → waitingForCandidateResponse: true
   → nextQuestion: короткий ответ (1-2 предложения) БЕЗ нового вопроса

e) Полный ответ на вопрос:
   → Задай 1-2 следующих вопроса естественно, БЕЗ оценок и комментариев
   → НЕ используй списки и нумерацию
   → НЕ давай оценки типа "Отлично!", "Интересный подход", "Это показывает системность"
   → Сразу переходи к следующему вопросу

ВАЖНО:
- Пиши КОРОТКО, как живой человек
- Задавай 1-2 вопроса за раз максимум
- Если кандидат задал вопрос - ответь кратко и жди
- Про голосовые упоминай ТОЛЬКО ОДИН РАЗ в начале

ФОРМАТ ОТВЕТА:
Верни JSON:
- analysis: краткая оценка (HTML: <p>, <strong>, <br>)
- shouldContinue: true/false
- reason: причина завершения (пустая строка если продолжаем)
- nextQuestion: следующее сообщение (пустая строка если завершаем)
- confidence: 0.0-1.0
- waitingForCandidateResponse: true если ждем ответа, иначе false
- isSimpleAcknowledgment: true если простое подтверждение, иначе false`;
  }
}

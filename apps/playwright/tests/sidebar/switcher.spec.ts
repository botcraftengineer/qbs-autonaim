...
test.describe("Switcher в сайдбаре", () => {
  ...
  test.describe("Отображение switcher", () => {
    ...
    test("показывает информацию об организации", async ({ page }) => {
      ...
      await expect(page.getByText(/участник/).first()).toBeVisible();
      await expect(page.getByText(/пространство/).first()).toBeVisible();
    });
  ...
  test.describe("Создание воркспейса", () => {
    ...
    test("создает новый воркспейс и переключается на него", async ({
      ...
      // Проверяем, что URL изменился (используем contains вместо regex для надежности)
      await page.waitForURL(
        (url) => url.pathname.includes(`/workspaces/${newWorkspaceSlug}`),
        { timeout: 10000 },
      );
  ...
  test.describe("Закрытие switcher", () => {
    test("закрывается при клике вне меню", async ({ page }) => {
      ...
      // Кликаем на body вместо main, чтобы избежать перехвата кликов элементами меню
      await page
        .locator("body")

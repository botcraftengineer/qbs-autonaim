...
export function DocsCodeBlock({ code, language = "bash", title, showLineNumbers = false }: DocsCodeBlockProps) {
            ...
            {showLineNumbers
              ? lines.map((line, i) => (
                  <div key={i} className="table-row">
                    <span className="table-cell select-none pr-4 text-right text-muted-foreground/50">{i + 1}</span>

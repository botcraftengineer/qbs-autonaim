...
export default function GigResponsesPage({ params }: PageProps) {
  ...
  const filteredResponses = React.useMemo(() => {
    ...
    return responses.filter((response) => {
      ...
      const matchesSearch = !searchQuery || 
        response.candidateName?.toLowerCase().includes(searchQuery.toLowerCase()) ||
      ...
      const matchesTab = activeTab === "all" || 
        ...
        (activeTab === "recommended" && response.hrSelectionStatus === "RECOMMENDED");
  ...
  const stats = React.useMemo(() => {
    ...
    return {
      ...
      recommended: responses.filter(r => r.hrSelectionStatus === "RECOMMENDED").length,
    };
  }, [responses]);
                    ...
                    <div className="flex items-start justify-between">
                      ...
                      <div className="flex items-center gap-2">
                        {response.hrSelectionStatus && (
                          <Badge variant={getHrStatusLabel(response.hrSelectionStatus).variant}>
                            {getHrStatusLabel(response.hrSelectionStatus).label}
                          </Badge>
                        )}
                        
                        <Badge variant={getStatusLabel(response.status).variant}>

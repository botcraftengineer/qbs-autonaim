...
- [ ] 2. Сервис генератора ссылок на интервью
  - [ ] 2.1 Реализовать класс `InterviewLinkGenerator` с методами: generateLink, validateLink, deactivateLink
...
- [ ] 3. Сервис парсера откликов
  - [ ] 3.1 Реализовать класс `ResponseParser` с методами: parseSingle, parseBulk, validateParsedData
...
- [ ] 4. tRPC роутер фриланс-платформ - Управление заданиями
  - [ ] 4.1 Создать роутер `freelancePlatforms` в tRPC API
    - Реализовать мутацию `createJob`: принимать title, description, requirements, platformSource, platformUrl
    - Реализовать запрос `getJobs`: возвращать все задания на фриланс-платформах для workspace, с фильтрацией по платформе
    - Реализовать запрос `getJobById`: возвращать одно задание со статистикой
    - Реализовать мутацию `updateJobStatus`: изменять статус задания (active/paused/closed)
    - _Требования: 9.1, 9.2, 9.3, 9.4, 9.6_

  - [ ]* 4.2 Написать тест свойств для валидации создания задания
    - **Свойство 22: Валидация создания задания**
    - **Проверяет: Требования 9.2**

  - [ ]* 4.3 Написать тест свойств для расчёта статистики
    - **Свойство 23: Расчёт статистики**
    - **Проверяет: Требования 9.4, 10.2**

- [ ] 5. tRPC роутер фриланс-платформ - Ссылки на интервью
  - [ ] 5.1 Реализовать эндпоинты ссылок на интервью
...
- [ ] 7. tRPC роутер фриланс-платформ - Ручной импорт
...
- [ ] 8. Фоновая задача AI-анализа
  - [ ] 8.1 Создать Inngest задачу: `freelance.response.analyze`
...
- [ ] 9. Генератор приглашений на интервью
...
- [ ] 11. Публичный портал интервью - Целевая страница
  - [ ] 11.1 Создать Next.js страницу: `/interview/[token]/page.tsx`
    - Валидировать токен с использованием tRPC запроса
    - Отображать название задания и информацию о компании
    - Показывать инструкции по интервью и примерное время
    - Рендерить форму информации о фрилансере (имя, email, телефон, профиль на платформе)
    - Добавить кнопку "Начать интервью"
    - _Требования: 2.3, 3.1, 3.2, 3.3_
...
- [ ] 12. Публичный портал интервью - UI веб-чата
  - [ ] 12.1 Создать компоненты интерфейса чата
    - Компонент списка сообщений с автопрокруткой
    - Компонент ввода текстового сообщения с кнопкой отправки
    - Индикатор "бот печатает..." при обработке
    - Отображение временных меток сообщений
    - Адаптивный дизайн для мобильных устройств
    - _Требования: 3.4, 3.5_

  - [ ] 12.2 Реализовать real-time коммуникацию
  ...
  - [ ] 12.4 Создать tRPC эндпоинты для веб-чата с буферизацией
    - Мутация `startWebInterview`: создать conversation с source='WEB', вернуть conversationId и первое сообщение
  ...
  - [ ] 12.5 Реализовать UI состояния буферизации
...
- [ ] 13. Генератор шортлиста
  - [ ] 13.1 Реализовать логику генерации шортлиста
  ...
  - [ ] 13.4 Создать tRPC эндпоинт: `getShortlist`
...
- [ ] 15. Фронтенд - UI управления заданиями
  - [ ] 15.1 Создать страницу списка заданий на фриланс-платформах
  ...
  - [ ] 15.2 Создать форму создания задания
